...
/**
 * 대기열 항목 도메인 모델과 JPA 엔티티 간의 매핑을 처리하는 MapStruct 인터페이스
 */
@Mapper(componentModel = "spring", 
        uses = {ConcertMapper.class},
        injectionStrategy = InjectionStrategy.CONSTRUCTOR)
public interface WaitingQueueEntryMapper {
    ...
    WaitingQueueEntry toDomain(WaitingQueueEntryJpaEntity entity);
     ...
     * @return 변환된 JPA 엔티티
     */
    WaitingQueueEntryJpaEntity toEntity(WaitingQueueEntry domain);
     ...
     */
    default WaitingQueueEntry toDomainById(Long id) {
        if (id == null) {
            return null;
        }
        return WaitingQueueEntry.builder().id(id).build();
    }
    ...
    void updateEntityFromDomain(WaitingQueueEntry domain, @MappingTarget WaitingQueueEntryJpaEntity entity);
}