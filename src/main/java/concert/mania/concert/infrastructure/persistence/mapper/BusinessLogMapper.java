...
/**
 * 비즈니스 로그 도메인 모델과 JPA 엔티티 간의 매핑을 처리하는 MapStruct 인터페이스
 */
...
public interface BusinessLogMapper {
     ...
     * @return 변환된 도메인 모델
     */
    BusinessLog toDomain(BusinessLogJpaEntity entity);
     ...
     * @return 변환된 JPA 엔티티
     */
    BusinessLogJpaEntity toEntity(BusinessLog domain);
     ...
     */
    default BusinessLog toDomainById(Long id) {
        if (id == null) {
            return null;
        }
        return BusinessLog.builder().id(id).build();
    }
    ...
    void updateEntityFromDomain(BusinessLog domain, @MappingTarget BusinessLogJpaEntity entity);
}