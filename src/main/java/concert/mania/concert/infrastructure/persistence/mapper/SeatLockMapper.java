...
/**
 * 좌석 잠금 도메인 모델과 JPA 엔티티 간의 매핑을 처리하는 MapStruct 인터페이스
 */
...
public interface SeatLockMapper {
     ...
     * @return 변환된 도메인 모델
     */
    SeatLock toDomain(SeatLockJpaEntity entity);
     ...
     * @return 변환된 JPA 엔티티
     */
    SeatLockJpaEntity toEntity(SeatLock domain);
     ...
     */
    default SeatLock toDomainById(Long id) {
        if (id == null) {
            return null;
        }
        return SeatLock.builder().id(id).build();
    }
    ...
    void updateEntityFromDomain(SeatLock domain, @MappingTarget SeatLockJpaEntity entity);
}